...
def check_exist_nodes_with_added(new_tops, index):
    ans = answers
    ghs = graphs
    mg = meta_graphs
    for i in range(len(new_tops.switched)):
        # for node in list(meta_graphs[index].graph.nodes):
        k = meta_graphs[index].graph.nodes
        for node in meta_graphs[index].graph.nodes:
            if isomorphism.is_isomorphic(new_tops.switched[i].graph, node.graph):  # and new_tops.switched[i] != node
                # meta_graphs[index].graph.remove_node(new_tops.switched[i])
                new_tops.switched.pop(i)
                # graphs[index].pop(i)
                graphs[index].remove(new_tops.switched[i])
...
def main():
    ...
    # degrees = [[4, 3, 3, 2, 2], [3, 2, 2, 1, 1, 1], [7, 6, 3, 3, 2, 2, 1, 1], [5, 4, 3, 3, 2, 1]]
    degrees = [[3, 2, 2, 1, 1, 1]]
