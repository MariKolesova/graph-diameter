...
def edge_switching(gr, index_meta_graph):  # (граф, номер мета)
    ...
    ghs = graphs
    mg = meta_graphs
    if gr not in graphs[index_meta_graph] or list(gr.graph.edges) == list(initial[index_meta_graph].graph.edges):
...
def main():
    ...
    for i in range(len(result_graphs_for_degrees)):
        initial.append(result_graphs_for_degrees[i])  # начальный, номер мета
        build_meta_graph(result_graphs_for_degrees[i], i)  # граф, номер в листе метаграф
