...
def build_meta_graph(initial_graph, index_meta_graph, level):
    mg2 = meta_graphs2
    cnt = count2
    ans = answers2
    g2 = graphs2
    if doing == 1:
        ...
        graphs2[index_meta_graph].append([])
...
def build_meta_for_2_flag(initial_g, index):  # (graph, 0)
    mg2 = meta_graphs2
    cnt = count2
    ans = answers2
    g2 = graphs2
    ...
    while continued:
        level = 0
        graphs.append([])
        build_meta_graph(initial_g, index, level)
...
def main():
    ...
    if doing == 1:
        for i in range(len(result_graphs_for_degrees)):
            build_meta_graph(result_graphs_for_degrees[i], i, 0)  # граф, номер в листе метаграфа
        for i in range(len(answers)):
            print('диаметр', result_graphs_for_degrees[i].degrees, '-', answers[i][1])
    else:
        gr = build_graph([4, 3, 3, 2, 1])
        result_graphs_for_degrees.append(gr)
        meta_graphs2.append(Graph())
        graphs2.append([])
        count2.append(False)
        answers2.append((result_graphs_for_degrees[i].degrees, 0))
        build_meta_for_2_flag(result_graphs_for_degrees[0], 0)
