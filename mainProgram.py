    ...
    for i in range(len(meta_graphs[index].graph.nodes)):
        for j in range(len(gr.switched)):
            if len(meta_graphs[index].graph.nodes) != 1:
                if not isomorphism.is_isomorphic(meta_graphs[index].graph.nodes[i], gr.switched[j]):
                    meta_graphs[index].graph.add_edge(meta_graphs[index].graph.nodes[i], gr.switched[j])
