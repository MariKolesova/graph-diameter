...
def add_edges(gr):
    for i in gr.graph.nodes:
        for k in range(i + 1, len(gr.degrees)):
            ...
            a = sum(gr.degrees[k + 1::])
            b = add_up(gr, k)
            res = a <= b
            if gr.graph.degree[i + 1] < gr.degrees[i + 1] and gr.graph.degree[k + 1] < gr.degrees[k + 1]  \
               and sum(gr.degrees[k + 1::]) > add_up(gr, k):
                gr.graph.remove_node(i, k)
    return gr


def add_up(gr, num):
    result = 0
    for i in range(num + 1, len(gr.degrees)):
        result += gr.degrees[i] - gr.graph.degree[i]
    return result
