...
def edge_switching(gr, index_meta_graph, g2):  # (граф, номер мета)
    ...
    if doing == 2:
        check_exist_nodes_with_added2(gr, index_meta_graph, g2)
        if get_diameter2(index_meta_graph, g2):
            calculate_diameter2(index_meta_graph, g2)
    if doing == 1:
...
def check_exist_nodes_with_added2(gr, index, g2):
    for i in range(len(meta_graphs2[index].graph.nodes)):
        for j in range(len(gr.switched)):
            if not in_graphs_index(gr.switched[j], index)\
                    and not isomorphism.is_isomorphic(list(meta_graphs2[index].graph.nodes)[i].graph, gr.switched[j].graph):
                meta_graphs2[index].graph.add_edge(list(meta_graphs2[index].graph.nodes)[i], gr.switched[j])
                graphs2[index].append(gr.switched[j])
...
def get_diameter2(index):
    all_nodes = list(meta_graphs2[index].graph.nodes)
    for node in all_nodes:
        if not node.flag_switching:
            return
    return True
...
def calculate_diameter2(number):
    mg2 = meta_graphs2
    ghs2 = graphs2
    cnt2 = count2
    ans2 = answers2
    answer = nx.diameter(meta_graphs2[number].graph)
    ########################################################################################
    #count2[number] = True
    #answers2[number] = (answers2[0][0], answer)
...
def build_meta_graph(initial_graph, index_meta_graph):
    ...
    while not count[index_meta_graph]:
        edge_switching(list(meta_graphs[index_meta_graph].graph.nodes)[i], index_meta_graph, 1)
        i += 1
...
def build_meta_graph2(initial_g, index, g2):
    mg2 = meta_graphs2
    cnt = count2
    ans = answers2
    g2 = [[initial_g]]
    meta_graphs2.append(Graph())
    meta_graphs2[index].graph.add_node(initial_g)

    i = 0
    while continue_iteration():
        edge_switching(list(meta_graphs2[index].graph.nodes)[i], index, g2)
        i += 1
...
def main():
    ...
    meta_graphs2 = []
    degrees = [[4, 3, 3, 2, 2], [3, 2, 2, 1, 1, 1], [7, 6, 3, 3, 2, 2, 1, 1]]
    ...
    if doing == 1:
        ...
        build_meta_graph2(result_graphs_for_degrees[0], 0, graphs2)
