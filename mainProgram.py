...
def make_part_meta_graph(gr, index_meta_graph):  # gr - (граф, лист переключенных)
    mg = meta_graphs
    edge_switching(gr, index_meta_graph)
    for g in gr[1]:
        make_part_meta_graph((g, []), index_meta_graph)  # (граф, лист переключенных), номер в листе метаграфа
...
def build_meta_graph(initial_graph, index_meta_graph):
    ...
    for gr in meta_graphs[index_meta_graph].graph.nodes:
        make_part_meta_graph((gr, []), index_meta_graph)  # (граф, лист переключенных), номер в листе метаграфа
    # nx.draw(graph)
    # plt.show()
