...
def edge_switching(gr, index_metaG):
    ghs = graphs
    mg = meta_graphs
    if not gr[0].flag_switching:
        gr[0].flag_switching = True
        meta_graphs[index_metaG].flag_switching = True
    else:
        return
    ...
    connect_graphs(gr, index_metaG)


def make_part_meta_graph(gr, index_metaG):
    edge_switching(gr, index_metaG)
...
def build_meta_graph(initial_graph, index_metaG):
    mg = meta_graphs
    ghs = graphs
    graphs[index_metaG].append(initial_graph[0])
    edge_switching(initial_graph, index_metaG)
    for gr in meta_graphs[index_metaG].graph.nodes:
        make_part_meta_graph((gr, []), index_metaG)
    # nx.draw(graph)
    # plt.show()
...
def main():
    global graphs # [] список всех вершин мета графа
    global meta_graphs
    graphs = []
    meta_graphs = []
