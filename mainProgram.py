...
def check_exist_nodes_with_added(gr, index):
    ...
    for i in range(len(meta_graphs[index].graph.nodes)):
        for j in range(len(gr.swithed)):
            if not isomorphism.is_isomorphic(meta_graphs[index].graph.nodes[i], gr.swithed[j]):  # and new_tops.switched[i] != node
                meta_graphs[index].graph.add_edge(meta_graphs[index].graph.nodes[i], gr.swithed[j])
                graphs[index].add(gr.switched[j])
